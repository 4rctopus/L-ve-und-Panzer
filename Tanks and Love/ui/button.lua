...
function ui.button( name, x, y, w, h, text, font )
    local state = {}
    state.hover = false
    state.down = {}
    ...
    -- check if mouse is hovering above button, adn set color accordingly
    ...
    if( text ~= nil ) then
        ...
        love.graphics.printf( text, x, y + ( h - font:getHeight( text ) ) / 2, w, "center" )
    end
